...
- Level-up hint (2026-09-19, unreleased, needs in-game check): `CardStats.TierDelta(turn, tier)` = games-weighted mean of card deltas over MINION cards with `TechLevel == tier` on that turn (≥30 games/card, ≥300 total). Label under the upgrade button (x=0.415W, y=0.255H, 16:9 assumption) compares next turn's (tier+1) vs (tier): diff ≥ +0.15 "▲ 레벨 유리", ≤ −0.15 "▼ 유지 유리", else "≈". Survivorship bias inflates all deltas; only the difference between tiers is the signal. Offline table (turn: T1..T6) e.g. turn 5: 0.88 0.86 0.91 1.11 1.35 1.32.
- Remaining ideas: quests when back in rotation; timewarp flag; pool-remaining estimate (deferred).
